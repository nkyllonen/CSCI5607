...
float cur_x_rotation = 0.0f;
float cur_y_rotation = 0.0f;
float cur_z_rotation = 0.0f;

float cur_x_position = 0.0f;
float cur_y_position = 0.0f;
float cur_z_position = 0.0f;
...
int main(int argc, char *argv[]){
  ...
  SDL_Window* window = SDL_CreateWindow("My ModelLoad_TR Program", 100, 100, screen_width, screen_height, SDL_WINDOW_OPENGL);
  ...
  ifstream modelFile;
	...
	//Little instructions menu
	printf("Controls:\
			\n Translation: arrow keys\
			\n Rotation:\
			\n  W/S - rotate along horizontal axis (screen x-axis)\
			\n  A/D - rotate along vertical axis (screen y-axis)\
			\n  Q/E - rotate along into/out-of axis (screen z-axis)\n\n");
	...
	while (!quit)
	{
		if (SDL_PollEvent(&windowEvent)) {
			...
			case SDL_KEYUP:
				...
				onKeyUp(windowEvent.key); //edit current translation and rotation parameters according to key press
				break;
			default:
				break;
			}//END polling switch
	  ...
	  //apply translations
	  model = glm::translate(model, glm::vec3(cur_x_position, cur_y_position, cur_z_position));

	  //apply rotations
	  model = glm::rotate(model, cur_x_rotation * 3.14f, glm::vec3(1.0f, 0.0f, 0.0f));
	  model = glm::rotate(model, cur_y_rotation * 3.14f, glm::vec3(0.0f, 1.0f, 0.0f));
	  model = glm::rotate(model, cur_z_rotation * 3.14f, glm::vec3(0.0f, 0.0f, 1.0f));
