...
/*=============================*/
void onKeyUp(SDL_KeyboardEvent& event, GLfloat verts[]);
...
int main(int argc, char *argv[]){
	...
	printf("Controls:\
			\n 0 - generate new random color for top vertex\
			\n 1 - generate new random color for right vertex\
			\n 2 - generate new random color for left vertex\n");
	...
	//Load the model data
	GLfloat vertices[] = { //We should read this from a file... for now lets hardcode it
	...
	while (!quit){
      if (SDL_PollEvent(&windowEvent)){
		  ...
		  case SDL_KEYUP:
			  ...
			  onKeyUp(windowEvent.key, vertices);
			  break;
		  default:
			  break;
		  }//END polling switch

		  //Reset anything that was changed (i.e. update vbo)
...
/*--------------------------------------------------------------*/
// onKeyUp : determine which key was pressed and what action
//				to take as a result
/*--------------------------------------------------------------*/
void onKeyUp(SDL_KeyboardEvent & event, GLfloat verts[])
{
	...
	case SDLK_KP_0:
	case SDLK_0:
		printf("\n0 key pressed - change top vertex\n");
		changeVertColor(0, verts);
		break;
	//CHANGE VERTEX 1 (bottom right)
	case SDLK_KP_1:
	case SDLK_1:
		printf("\n1 key pressed - change right vertex\n");
		changeVertColor(1, verts);
		break;
	//CHANGE VERTEX 2 (bottom left)
	case SDLK_KP_2:
	case SDLK_2:
		printf("\n2 key pressed - change left vertex\n");
		changeVertColor(2, verts);
		break;
...
/*--------------------------------------------------------------*/
// changeVertColor : changes color indices corresponding to
//						given v_index
/*--------------------------------------------------------------*/
void changeVertColor(int v_index, GLfloat verts[])
{
